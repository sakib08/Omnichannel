...
class Ppros_Synchronized_Messaging_Engine_Messenger_Pipe extends Ppros_Synchronized_Messaging_Engine_Channel_Pipe_Base {
    ...
    private function process_message( array $event, array $cfg ): void {
        ...
        if ( is_wp_error( $conversation_id ) ) {
            error_log( '[SME Messenger] DB error: ' . $conversation_id->get_error_message() );
            return;
        }
        ...
        do_action( 'sme_inbound_message_received', $conversation_id, 'messenger', array(
    ...
    protected function send_message( string $recipient_id, string $text, array $cfg = array() ) {
        ...
        if ( '' === $token ) {
            return new \WP_Error(
                ...
                __( 'Messenger page access token is not configured.', 'synchronized-messaging-engine' )
            );
        }
        ...
        if ( is_wp_error( $result ) ) {
            return new \WP_Error(
                ...
                sprintf( __( 'Messenger API error: %s', 'synchronized-messaging-engine' ), $result->get_error_message() ),
                array( 'status' => 502 )
            );
        }
